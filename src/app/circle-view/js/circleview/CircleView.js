...
$(document).ready(function(){
    ...
    function displayObjectsFromDatabase() {
    	$.each(objectDatabase, function(i, v) {
			genericAdd(v.title,v.image,v.url);
		});		
    }
	...
	function genericAdd(object_title,object_image,object_url) {
		var object_url_html='';
		...
		attachObject(object_image_html,object_url_html)
		
		
	}
	
	function attachObject(object_image_html,object_url_html) {
		var $object = $('<div class="object draggableObjects">'+object_image_html+object_url_html+'</div>');
		$('#objectBox').prepend( $object ).masonry( 'reload' );			
		$object.draggable({
			cancel: "a.ui-icon", 
			revert: "invalid", 
